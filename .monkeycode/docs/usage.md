...
同步工具现在根据表的数据特性自动选择合适的同步策略：

#### 静态数据（无时间字段）
- **适用场景**：配置表、字典表、产品表等数据变化不大的表
- **同步方式**：按主键增量（UPSERT 模式）
- **配置方法**：时间字段留空，勾选"启用"即可

#### 动态数据（有时间字段）
- **适用场景**：订单表、日志表、交易表等持续产生新数据的表
- **同步方式**：按时间范围增量（默认最近 3 天）
- **配置方法**：
  1. 填写"时间字段"（如 UpdateTime、CreateTime）
  2. 勾选"启用时间范围"
  3. 设置"同步范围天数"（默认 3 天）
...
**首次同步逻辑**：
- 动态数据：首次同步全量历史数据
- 后续同步：按配置的范围天数同步（如最近 3 天）
- 静态数据：按主键增量，只同步新增或变更的记录
...
**添加表**：
1. 点击"从数据库加载"，自动连接源库并加载所有表
2. 或点击"添加表"，从表选择对话框中手动选择
3. 表选择对话框支持搜索过滤
...
| 时间字段 | 可选，有时间字段的表为动态数据 |
| 启用时间范围 | 勾选后按时间范围同步（动态数据） |
| 启用 | 勾选后参与同步 |
| 状态 | 待同步/同步中/成功/部分失败 |

**动态数据 vs 静态数据配置示例**：

| 表名 | 主键 | 时间字段 | 启用时间范围 | 类型 | 说明 |
|------|------|----------|--------------|------|------|
| Products | Id | (留空) | ☐ | 静态 | 按主键增量 |
| Orders | OrderId | CreateTime | ☑ | 动态 | 按最近 3 天同步 |
| Configs | Id | (留空) | ☐ | 静态 | 全量 +UPSERT |
