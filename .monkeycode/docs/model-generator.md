...
| 测试连接按钮 | 验证连接是否可用 |
| 命名空间输入框 | 设置默认命名空间 |
| 包含视图复选框 | 控制是否显示视图（显示 `[视图]` 标识） |
| 搜索框 | 筛选表名（支持模糊匹配） |
| 加载表按钮 | 从数据库加载表列表 |
| 表列表 | 勾选要生成的表，表/视图前有 `[表]`/`[视图]` 标识 |
...
A: 支持。v1.1+ 版本已支持视图生成。表列表中会显示 `[表]` 或 `[视图]` 前缀标识，可通过"包含视图"复选框控制是否显示视图。
...
- 新增：视图支持，可生成视图对应的 Model 类
- 新增："包含视图"复选框，控制是否显示视图
- 新增：表列表显示 `[表]` / `[视图]` 前缀标识
- 修复：UI 布局 row 冲突问题
