...
namespace FastData.Demo.Repositories
{
    ...
    public class UserRepository : IUserRepository
    {
        ...
        {
            return await Task.FromResult(Db.Use(_key).Query<AppUser>(u => u.Id > 0)
                .OrderBy(u => u.Id)
                .ToList<AppUser>());
        }
        ...
        {
            return await Task.FromResult(Db.Use(_key).First<AppUser>(u => u.Id == id));
        }

        public async Task<List<AppUser>> GetByDepartmentAsync(string department)
        {
            return await Task.FromResult(Db.Use(_key).List<AppUser>(u => u.Department == department));
        }

        public async Task<List<AppUser>> GetActiveUsersAsync()
        {
            return await Task.FromResult(Db.Use(_key).List<AppUser>(u => u.IsActive == true));
        }
        ...
        public async Task<(bool Success, string Message)> AddAsync(AppUser user)
        {
            ...
            user.IsActive = true;
            var result = await Task.FromResult(Db.Use(_key).Add(user));
        ...
        public async Task<(bool Success, string Message)> UpdateAsync(AppUser user)
        {
            ...
            var result = await Task.FromResult(Db.Use(_key).Update(user, a => new { a.UserName, a.Email, a.Phone, a.Age, a.Department, a.Salary, a.UpdateTime }));
            return (result.IsSuccess, result.Message);
        }

        public async Task<(bool Success, string Message)> DeleteAsync(int id)
        {
            var result = await Task.FromResult(Db.Use(_key).Delete<AppUser>(a => a.Id == id));
            return (result.IsSuccess, result.Message);
        }
    ...
    public class OrderRepository : IOrderRepository
    {
        ...
        {
            return await Task.FromResult(Db.Use(_key).Query<AppOrder>(o => o.Id > 0)
        ...
        {
            return await Task.FromResult(Db.Use(_key).First<AppOrder>(o => o.Id == id));
        }

        public async Task<List<AppOrder>> GetByUserIdAsync(int userId)
        {
            return await Task.FromResult(Db.Use(_key).Query<AppOrder>(o => o.UserId == userId)
        ...
        {
            return await Task.FromResult(Db.Use(_key).List<AppOrder>(o => o.Status == status));
        }
        ...
        public async Task<(bool Success, string Message, string OrderNo)> AddAsync(AppOrder order)
        {
            ...
            var result = await Task.FromResult(Db.Use(_key).Add(order));
        ...
        public async Task<(bool Success, string Message)> UpdateStatusAsync(int id, int status)
        {
            ...
            var result = await Task.FromResult(Db.Use(_key).Update(order, a => new { a.Status, a.PayTime }));
            return (result.IsSuccess, result.Message);
        }
    }
}
