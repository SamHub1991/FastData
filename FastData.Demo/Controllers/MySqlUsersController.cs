...
namespace FastData.Demo.Controllers
{
    ...
    /// </summary>
    [ApiController]
    [Route("api/mysql-users")]
    public class MySqlUsersController : ControllerBase
    {
        private const string DbKey = "MySql";
        ...
        public async Task<ActionResult<List<AppUser>>> GetByDepartment(string department)
        {
            try
            {
                var users = await Task.FromResult(Db.Use(DbKey).List<AppUser>(u => u.Department == department));
                return Ok(users);
            }
            catch (Exception ex)
            {
