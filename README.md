...
**使用流程**：
1. 配置源库、目标库、中间库连接（或使用"数据库配置"Tab 页保存的连接）
2. 从源库加载表，选择要同步的表
...
- **实时日志**：同步过程实时显示进度和错误
- **数据库连接管理**：保存/测试/快速加载数据库连接
- **数据补录**：手动补录指定时间段的历史数据
- **跨库同步**：支持不同数据库类型之间的数据同步
- **自动重试**：网络异常自动重试 3 次（间隔 5 秒）
- **共享日志**：所有操作统一日志，支持导出文件
...
当前构建状态：`Build succeeded. 0 Warning(s), 0 Error(s)`
